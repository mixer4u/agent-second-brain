...
Эта система использует **MCP (Model Context Protocol)** — открытый стандарт от Anthropic для подключения AI к внешним сервисам.

> **Рекомендуем:** Используйте [mcp-cli](docs/mcp-cli-setup.md) для стабильного подключения. Это обёртка с connection pooling и auto-retry — соединения не отваливаются.

По умолчанию настроен Todoist, но вы можете подключить любой сервис, для которого есть MCP-сервер:
